...
//         {/* Left */}
//         <div className="lg:col-span-7 space-y-3 sm:space-y-4 lg:space-y-5 order-2 lg:order-1">
//           {/* Slide Indicators */}
//           <div className="flex space-x-2 sm:space-x-3 mb-2 sm:mb-3 justify-center lg:justify-start">
...
//           {/* Search Bar - Positioned above title */}
//           <div
//             className={`px-4 lg:px-0 mb-4 sm:mb-5 lg:mb-6 ${
//               isLoaded
...
//                 ? "animate-[fadeInUp_0.8s_ease-out_forwards]"
//                 : "opacity-0"
//             } ${searchFocused ? "lg:opacity-100 opacity-0 pointer-events-none lg:pointer-events-auto" : "opacity-100"}`}
...
export default function EnhancedHeroSection() {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <div 
        className="relative z-40 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8 items-center"
        style={{
          // When search is focused, reduce z-index of content below search
          zIndex: searchFocused ? 997 : 40
        }}
      >
        {/* Left */}
        <div className="lg:col-span-7 space-y-3 sm:space-y-4 lg:space-y-5 order-2 lg:order-1">
              ...
              <button
                ...
                className={`relative overflow-hidden transition-all duration-500 rounded-full ${
                  idx === currentSlide
                    ? "w-8 sm:w-10 lg:w-12 h-2 sm:h-2.5 lg:h-3 bg-amber-400 shadow-lg shadow-amber-400/50"
                    : "w-2 sm:w-2.5 lg:w-3 h-2 sm:h-2.5 lg:h-3 bg-white/30 hover:bg-white/50 hover:scale-125"
                } ${searchFocused ? 'pointer-events-none opacity-50' : ''}`}
                aria-label={`Go to slide ${idx + 1}`}
              >
                {idx === currentSlide && (
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-300 to-orange-400 animate-pulse" />
          ...
          <div
            ...
            style={{ 
              animationDelay: "0.1s",
              position: 'relative',
              zIndex: 1000  // Ensure search is always on top
            }}
          >
            <SearchBar
              isHeroVersion
        ...
        {/* Right - Weather and Stats */}
        <div 
          className={`lg:col-span-5 space-y-4 sm:space-y-6 order-1 lg:order-2 px-4 lg:px-0 transition-opacity duration-300 ${
