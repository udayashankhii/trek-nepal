...
// import {searchParams, useSearchParams, useNavigate,navigate } from "react-router-dom";
// import {
//   Plus,
//   Minus,
//   Calendar,
...
// import { fetchTrekHero } from "../../api/trekService.js";
...
//     // Form state (identical to your original)
...
//       setError(null);
//       try {
//         const data = await fetchTrekHero(trekId);
//         if (!data) throw new Error("No hero data");
//         setHero(data.hero);
//         setTrekData(data.trek);
//       } catch (err) {
...
//   if (!trekData) return <p>No data found.</p>;
//   // country list


//   // Map API shapes -> fallbacks
//   // const mainTrek = trekData?.main ?? trekData ?? null;
//  // Calculate dynamic endDate based on trek duration (if start provided)
//   useEffect(() => {
//     if (startDate) {
//       const days = parseDurationDays(mainTrek?.duration);
...
//     cover_image: "/images/everest-cover.jpg",
//   };
     



//   const highlights = mainTrek?.highlights ?? trekData?.highlights ?? mainTrek?.itinerary ?? [];

//   // const hero = {
//   //   imageUrl:
//   //     trekData?.hero?.imageUrl ??
//   //     mainTrek?.heroImage ??
//   //     mainTrek?.cover_image ??
//   //     trekData?.heroImage ??
//   //     "/images/everest-base-camp-hero.jpg",
//   //   title: trekData?.hero?.title ?? mainTrek?.name ?? mainTrek?.title ?? "Your Trip",
//   //   subtitle: trekData?.hero?.subtitle ?? mainTrek?.subtitle ?? "",
//   //   duration: mainTrek?.duration ?? `${parseDurationDays(mainTrek?.duration)} Days`,
//   //   difficulty: mainTrek?.difficulty ?? mainTrek?.level ?? "",
//   //   location: mainTrek?.location ?? mainTrek?.region ?? undefined,
//   // };

// // const hero = useMemo(() => {
// //   if (!trekData) return null;
// //   const trek = trekData.trek ?? trekData;
// //   return {
// //     imageUrl: trekData.hero?.imageUrl ?? mainTrek.heroImage,
// //     title: trekData.hero?.title ?? mainTrek.title,
// //     subtitle: trekData.hero?.subtitle ?? "",
// //     duration: trekData.hero?.duration ?? mainTrek.duration,
// //     difficulty: trekData.hero?.difficulty ?? mainTrek.difficulty,
// //     location: trekData.hero?.location ?? mainTrek.region,
// //   };
// // }, [trekData]);
...
//   // Pricing logic (robust)
//   const priceFromParams = Number(searchParams.get("price")) || null;
//   const apiPrice =
//     mainTrek?.booking_card?.base_price ??
//     mainTrek?.base_price ??
//     trekData?.base_price ??
//     null;

//   const basePrice = apiPrice ? parseFloat(apiPrice) : priceFromParams || 1525;
...
//   const changeLead = (e) => setLead((l) => ({ ...l, [e.target.name]: e.target.value }));
...
//   // From here the JSX mirrors your original layout and styling
//   return (
//     <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
// {/* <BookingPageHero hero={trekData.hero} trek={trekData.trek} /> */}
// <BookingPageHero hero={hero} trek={trekData} />


// {/* <BookingPageHero hero={hero} /> */}
...
//                   <p className="text-gray-600">Complete your booking in one simple form</p>
//                 </div>
...
//                         <Calendar className="absolute left-4 top-4 w-5 h-5 text-gray-400" />
//                       </div>
//                       <p className="text-xs text-gray-500">Automatically calculated ({hero.duration})</p>
//                     </div>
//                   </div>
...
//                         <span className="text-gray-500">{travellers === 1 ? "person" : "people"}</span>
//                       </div>
...
//                       </div>
//                       {lead.email && !emailValid && <p className="text-sm text-red-500">Please enter a valid email address</p>}
//                     </div>
...
//                       </div>
//                       {lead.phone && !phoneValid && <p className="text-sm text-red-500">Please enter a valid phone number with country code</p>}
//                     </div>
//                   </div>

//                   {/* Country */}
//                   <div className="space-y-2 mb-6">
//                     <label className="block text-sm font-semibold text-gray-700">Country (Optional)</label>
//                     <div className="relative">
//                       <select
//                         name="country"
//                         value={lead.country}
...
//                     <div className="space-y-2">
//                       <label className="block text-sm font-semibold text-gray-700">Dietary Requirements</label>
//                       <input
//                         name="dietaryReqs"
...
//                   <div className="space-y-2">
//                     <label className="block text-sm font-semibold text-gray-700">Comments or Questions</label>
//                     <textarea
//                       name="comments"
//                       value={preferences.comments || ""}
//                       onChange={(e) => setPreferences((p) => ({ ...p, comments: e.target.value }))}
//                       rows={4}
...
//                     <h3 className="text-xl font-bold text-gray-900">Nepal Travel Times</h3>
//                   </div>

//                   <p className="text-sm text-gray-600 mb-6">
//                     If you need assistance with travel arrangements to/from Nepal, please provide your preferred times.
//                   </p>

//                   <div className="grid md:grid-cols-2 gap-6">
//                     <div className="space-y-2">
//                       <label className="block text-sm font-semibold text-gray-700">Departure Time to Nepal</label>
//                       <div className="relative">
//                         <input
//                           type="time"
//                           value={departureTime}
...
//                       </div>
//                       {departureTime && <p className="text-xs text-purple-600 font-medium">Nepal Time: {formatNepalTime(departureTime)}</p>}
//                     </div>

//                     <div className="space-y-2">
//                       <label className="block text-sm font-semibold text-gray-700">Return Time from Nepal</label>
//                       <div className="relative">
//                         <input
//                           type="time"
//                           value={returnTime}
...
//                         <Clock className="absolute left-4 top-4 w-5 h-5 text-gray-400" />
//                       </div>
//                       {returnTime && <p className="text-xs text-purple-600 font-medium">Nepal Time: {formatNepalTime(returnTime)}</p>}
//                     </div>
//                   </div>
//                 </div>
...
//                     }`}
//                   >
//                     {formValid ? "Complete Booking & Pay Now" : "Please Complete Required Fields"}
//                   </button>
//                 </div>
//               </div>
//             </form>
...
//                 <h3 className="font-bold text-xl mb-2">{mainTrek?.name ?? mainTrek?.title}</h3>
//                 <div className="flex items-center space-x-2 text-sm">
//                   <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
//                   <span>{mainTrek?.rating ?? "N/A"}</span>
//                   <span>({mainTrek?.reviews ?? 0} reviews)</span>
//                 </div>
//               </div>
...
//                   <p className="text-xs text-gray-600">Secure booking with 256-bit SSL encryption</p>
//                 </div>
//               </div>
//             </div>
//           </aside>
...
export default function SinglePageBookingForm() {
  ...
  // Declare mainTrek before any hooks or JSX that use it
  const mainTrek = {
    name: "Everest Base Camp Trek",
    title: "Everest Base Camp Trek",
    duration: "14 Days",
  ...
  const highlights =
    mainTrek?.highlights ?? trekData?.highlights ?? mainTrek?.itinerary ?? [];
                ...
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      {departureTime && (
                        <p className="text-xs text-purple-600 font-medium">Nepal Time: {formatNepalTime(departureTime)}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {returnTime && (
                        <p className="text-xs text-purple-600 font-medium">Nepal Time: {formatNepalTime(returnTime)}</p>
                      )}
                    </div>
                  </div>
                </div>
