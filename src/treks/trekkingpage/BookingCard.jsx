...
import { getAccessToken } from "../../api/auth/auth.api.js"; // ✅ Import auth helper
...
function BookingCard({
  ...
  // ✅ FIXED: Handler for Customize Trek with auth check
  const handleCustomize = () => {
    if (!trekIdentifier) return;

    const isAuthenticated = !!getAccessToken();
    const customizeUrl = `/customize-trek?trek_id=${trekIdentifier}`;
    ...
    if (isAuthenticated) {
      // User is logged in, go directly to customize page
      navigate(customizeUrl, { state: customizeState });
    } else {
      // User not logged in, show modal login
      navigate("/login", {
        state: {
          backgroundLocation: location, // ✅ This triggers the modal
          next: customizeUrl,
          customizeState: customizeState, // Pass the state to restore after login
        },
      });
    }
  };
